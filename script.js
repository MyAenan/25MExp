...
$(document).ready(function() {
	$.getJSON('expdata.json', function(data) {
		...
		$.getJSON('preprocessed.json', function(data) {
			prepdata = data;
	...
	$('#entryform input').on('change',function() {
		...
		} else if( targExp < 100023 || targExp > 25000000 ) {
			createAlert('error','The Target Exp must be 25 Million only');
	    } else if( targExp == currExp ) {
			createAlert('success','You are already at the Target Exp!');
		} else if( currExp < 100000 || currExp > 24999977 ) {
			createAlert('error','This calculator only works for 100,000 to 24, 999,977 Exp');
